          ...
          Function {funProjection=pr} ->
            case pr of
              ...
              Just Projection{projProper=Just recN} -> do
                ...
                inTopContext $ do
                  El s preTy <- reconstructParametersInType' (etaExpandButInProjAction (Just recN) eta) tExpand
...
-- The Maybe QName argument is not to etaExpand in the parameters of a projection.
etaExpandType :: DkModuleEnv -> Maybe QName -> Type -> TCM Type
etaExpandType eta recN (El s (Pi a@Dom{unDom=El sa u} b)) = do
  uu <- checkInternal' (etaExpandButInProjAction recN eta) u (sort sa)
  let dom = El sa uu
  addContext (absName b,a) $ do
    codom <- etaExpandType eta recN (absBody b)
    escapeContext 1 $ return $ El s (Pi a{unDom=dom} (Abs (absName b) codom))
etaExpandType eta recN (El s u) = do
  uu <- checkInternal' (etaExpandButInProjAction recN eta) u (sort s)
  return $ El s uu
...
etaExpButTheRecordArgs eta u@(El s (Pi a b)) 0 recN = do
  ...
  addContext (absName b,a) $ do
    codom <- etaExpandType eta (Just recN) (absBody b)
...
etaExpansionDecl eta n nbPars ConHead{conName = cons} l = do
  ...
  where
    ...
    constructRhsFields t args (u:tl) = do
      ...
      prEta <- etaExpansion eta Nothing tyRes (Var 0 [Proj ProjSystem u])
...
etaExpandButInProjAction :: Maybe QName -> QName -> Action TCM
etaExpandButInProjAction r eta = defaultAction { preAction = \y -> etaContractFun , postAction = etaExpansion eta r}

etaExpandAction :: QName -> Action TCM
etaExpandAction = etaExpandButInProjAction Nothing
...
etaExpansion :: QName -> Maybe QName -> Type -> Term -> TCM Term
etaExpansion eta recN t u = do
  reportSDoc "toDk.eta" 3 $ (text "Eta expansion of" <+>) <$> AP.prettyTCM u
  reportSDoc "toDk.eta" 3 $ (text "   of type" <+>) <$> AP.prettyTCM t
  case recN of
    Just r -> do
      reportSDoc "bla" 2 $ return $ text "recN is Just"
      case u of
        Def n _ -> do
          reportSDoc "bla" 2 $ return $ text "Def First"
          recFound <- maybe Nothing projProper <$> (isProjection n)
          if (recFound == recN)
          then do
            reportSDoc "bla" 2 $ return $ text "Le bon cas !!!"
            return u
          else defCase
        Var _ [Proj _ n] -> do
          reportSDoc "bla" 2 $ return $ text "Proj First"
          recFound <- maybe Nothing projProper <$> (isProjection n)
          if (recFound == recN)
          then do
            reportSDoc "bla" 2 $ return $ text "Le bon cas !!!"
    ...
    defCase =
      case unEl t of
        ...
        Pi (a@Dom{domInfo=info}) b -> do
          ...
          aExp <- etaExpandType eta recN (unDom a)
          addContext (n,a) $ do
            ...
            body <- etaExpansion eta recN (absBody b) appli
